'''
File: train.py
Author: (primarily) Will Schwarzer (schwarzerw@carleton.edu)
...
def parse_args():
    ...
    parser.add_argument('--del-discrim-graph', type=bool, default=True,
                        help='whether or not to delete the discriminator\'s autograd data')
    parser.add_argument('--embedding-file', type=str, default='caption_embedding.pkl',
...
def train(G, D, epoch, loader, txt_loader, G_optim, D_optim, val=False):
    ...
    for batch_idx, (batch, txt_batch) in enumerate(zip(loader, txt_loader)):
        ...
        if(epoch == args.epochs - 1 or epoch == args.epochs - 2 or epoch % 10 == 0) and batch_idx == 0:
            for image_real, image_fake, txt_new in zip(img, fake, txt_batch[1]):
                ...
                filename = os.path.join(model_dir, "real_" + str(epoch) + "-" + theString + ".png")
                plt.savefig(filename)
                plt.imshow(image_fake)
                plt.title("Fake " + theString)
                filename = os.path.join(model_dir, "fake_" + str(epoch) + "-" + theString + ".png")
                plt.savefig(filename)
                """
                ...
                """
                filename = os.path.join(model_dir, "text_" + str(epoch) + "-" + theString + ".txt")
            ...
            plt.imshow(image_real)
            plt.title("Real " + theString)
            plt.savefig(model_dir + "real_" + str(epoch) + "-"+ theString + ".png")
            plt.imshow(image_fake)
            plt.title("Fake " + theString)
            plt.savefig(model_dir +"fake_" + str(epoch) + "-" + theString + ".png")
            """
            Might get error saying:
            Clipping input data to the valid range
            for imshow with RGB data ([0..1] for floats or [0..255] for integers).
            """
            with open(model_dir + 'text_' + str(epoch) + "-"+ theString + '.txt', 'w') as txt_file:
                txt_file.write(txt_new)
    ...
    drivePath = os.path.join(args.out_dir, 'checkpoints/')
    checkpoint = {
        'epoch': epoch,
